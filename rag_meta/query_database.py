    ...
    def search_table(
        ...
        try:
            ...
            for row in response.data:
                if not row.get(config["embedding_column"]):
                    continue
                    
                # Calculate cosine similarity
                similarity = self._cosine_similarity(
                    query_embedding, 
                    row[config["embedding_column"]]
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Example searches
        print("=== Searching for AI/ML mentions ===")
        results = searcher.search("artificial intelligence machine learning", 
                                 SearchFilters(limit=5, min_similarity=0.75))
        searcher.print_results(results)
        
        print("\n=== Searching GTLB earnings calls ===")
        results = searcher.search_by_ticker("revenue growth", "GTLB", limit=3)
        searcher.print_results(results)
